...
#define _sptr_min_pad (2 * sizeof(size_t))
...
#define _SPTR_LENGTH_INDEX (-1)
#define _SPTR_SIZE_INDEX (-2)
...
void* realloc_sptr(void* sptr, size_t nitems) {
    if (sptr == NULL) {
        ...
        size_t allocate;
        if (_sptr_multiply_overflow_check(nitems, ((size_t*) sptr)[_SPTR_SIZE_INDEX], &allocate)) {
            return NULL;
        }
        if (_sptr_addition_overflow_check(allocate, pad, &allocate)) {
            return NULL;
            ...
            new_sptr = (size_t*) (((char*) new_sptr) + pad);
...
size_t sptr_size(void* sptr) {
    if (sptr == NULL) {
        ...
        size_t size;
        if (_sptr_multiply_overflow_check(
                ((size_t*) sptr)[_SPTR_LENGTH_INDEX], ((size_t*) sptr)[_SPTR_SIZE_INDEX], &size
        )) {
            return 0;
        }
        return size;
...
size_t sptr_item_size(void* sptr) {
    ...
    return ((size_t*) sptr)[_SPTR_SIZE_INDEX];
}
...
size_t sptr_length(void* sptr) {
    ...
    return ((size_t*) sptr)[_SPTR_LENGTH_INDEX];
}
...
void* add_realloc_sptr(void* sptr, size_t additional_items) {
    if (sptr == NULL) {
        ...
        size_t pad = (size_t) ((char*) sptr - (char*) meta);
        size_t* new_sptr;
        if (_sptr_addition_overflow_check(((size_t*) sptr)[_SPTR_LENGTH_INDEX], additional_items, &new_length)) {
            new_length = (size_t) - 1;
        }
        if (_sptr_multiply_overflow_check(new_length, ((size_t*) sptr)[_SPTR_SIZE_INDEX], &allocate)) {
            return NULL;
        }
        if (_sptr_addition_overflow_check(allocate, pad, &allocate)) {
            return NULL;
...
void* subtract_realloc_sptr(void* sptr, size_t removed_items) {
    if (sptr == NULL) {
        ...
        size_t pad = (size_t) ((char*) sptr - (char*) meta);
        size_t* new_sptr;
        if (_sptr_subtraction_underflow_check(((size_t*) sptr)[_SPTR_LENGTH_INDEX], removed_items, &new_length)) {
            new_length = 0;
        }
        if (_sptr_multiply_overflow_check(new_length, ((size_t*) sptr)[_SPTR_SIZE_INDEX], &allocate)) {
            return NULL;
...
void* resize_sptr(void* sptr, size_t new_size) {
    if (new_size == 0) {
        ...
        size_t* meta = (size_t*) _get_sptr_meta(sptr);
        size_t overall_size;
        if (_sptr_multiply_overflow_check(((size_t*) sptr)[_SPTR_LENGTH_INDEX], ((size_t*) sptr)[_SPTR_SIZE_INDEX], &overall_size)) {
            return NULL;
        } else {
            ...
            if (new_pad < old_pad) {
                ...
                memmove(((char*) meta) + new_pad - _sptr_min_pad, ((char*) meta) + old_pad - _sptr_min_pad, overall_size);
            ...
            } else if (new_pad > old_pad) {
                size_t allocate;
                overall_size = new_length * new_size;
                if (!_sptr_addition_overflow_check(overall_size, new_pad, &allocate)) {
                    char* new_sptr = (char*) realloc(meta, allocate);
                    if (new_sptr != NULL) {
                        memmove(new_sptr + new_pad - _sptr_min_pad, new_sptr + old_pad - _sptr_min_pad, overall_size);
                        return new_sptr + new_pad;
                    }
                }
            }
...
void* sptr_multiply_allocation(void* sptr, double multiplier) {
    if (sptr == NULL || multiplier < 0) {
        ...
        size_t pad = (size_t) ((char*) sptr - (char*) meta);
        if (multiplier) {
            length = ((size_t*) sptr)[_SPTR_LENGTH_INDEX];
        ...
        if (_sptr_multiply_overflow_check(((size_t*) sptr)[_SPTR_SIZE_INDEX], length, &allocate)) {
            return NULL;
        } else if (_sptr_addition_overflow_check(allocate, pad, &allocate)) {
            return NULL;
        }
...
static void* _get_sptr_meta(void* sptr) {
    ...
    return ((char*) sptr) - _sptr_get_pad(sptr_item_size(sptr));
}
...
static size_t _sptr_get_pad(size_t size) {
    return size ? _sptr_min_pad + ((_sptr_min_pad % size) ? (size - (_sptr_min_pad % size)) : 0) : _sptr_min_pad;
}
